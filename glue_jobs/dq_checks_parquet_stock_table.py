...
# this check counts the number of NULL rows in the close column
# if any rows are NULL, the check returns a number > 0
NULL_DQ_CHECK = f"""
SELECT 
    SUM(CASE WHEN close IS NULL THEN 1 ELSE 0 END) AS res_col
FROM "de_project_database"."nvda_stocks_data_parquet_tbl"
;
"""
